...
export TEXTDOMAIN=biglinux-games-settings
...
# change the state
  ...
  if [[ "$XDG_CURRENT_DESKTOP" == *"KDE"* ]] || [[ "$XDG_CURRENT_DESKTOP" == *"Plasma"* ]];then
    if [ "$state" == "true" ]; then
      ...
      rm $HOME/.config/biglinux-games-settings/effectsEnable
      for effect in ${effects[@]}; do
        mkdir -p $HOME/.config/biglinux-games-settings
        echo $effect >> $HOME/.config/biglinux-games-settings/effectsEnable
      ...
      effects=$(cat $HOME/.config/biglinux-games-settings/effectsEnable)
      for effect in ${effects[@]}; do
